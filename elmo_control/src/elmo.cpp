...
static void publishDiagonstics(RealtimePublisher<diagnostic_msgs::DiagnosticArray>& publisher,
                               elmo_control::ElmoHardwareInterface* robot = NULL){
  if(publisher.trylock()){
    ...
    if(robot!=NULL){
      ...
      int n_joint = robot->getInputActualValueToStatus(joint_names,hardware_ids,position_actual_values,velocity_actual_values);
      for(int i=0;i<n_joint;i++){
        diagnostic_updater::DiagnosticStatusWrapper status;
        status.hardware_id = hardware_ids[i];
        status.addf("Position actual value", "%d", position_actual_values[i]);
...
void *controlLoop(void */*unused_param*/){
  ...
  ros::NodeHandle nh;
  RealtimePublisher<diagnostic_msgs::DiagnosticArray> publisher(nh, "/diagonostics",2);
  ...
  double period_in_secs = 1e+9 * g_options.period_;
  ...
  elmo_control::ElmoHardwareInterface robot(g_options.iface_, g_options.simulation_);

  // Create controller manager
  controller_manager::ControllerManager cm(&robot);

  // Publish one-time before entering real-time to pre-allocate message vectors
  publishDiagonstics(publisher);
  ...
  while(!g_quit){
    ...
    if ((end - last_published) > 1.0){
      publishDiagonstics(publisher, &robot);
      last_published = end;
    }
...
int main(int argc, char *argv[]){
  ...
  if(mlock_all == false){
    ...
    if(g_options.simulation_ == false){
      perror("Exiting .. real robots needs mlockall to run in realtime");
      exit(EXIT_FAILURE);
    }
  }
