...
VelIn readInputs(){
  ...
  input.status =     *(uint16 *)(map+0);
  input.operation_mode = *(uint8 *)(map+2);
  input.digital_inputs = *(uint32 *)(map+3);
  input.current =        *(uint16 *)(map+7);
  input.position =       *(uint32 *)(map+9);
  input.velocity =       *(uint32 *)(map+13);
  input.digital_suppply =*(uint16 *)(map+17);
  input.dc_link_voltage =*(uint32 *)(map+19);
  return input;
}*/
